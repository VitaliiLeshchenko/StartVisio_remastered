  ...
  FROM (SELECT DISTINCT th.task_id,
                        CASE
                          ...
                          WHEN im.cd_master_id = '3001' AND im.sale_grp IN ('TCS', 'TCSR') THEN
                           'USB'
                          WHEN im.cd_master_id = '2001' AND substr(im.size_desc, 1, 7) = 'TRANSIT' THEN
                           'TRANSIT'
                          ...
                          WHEN im.cd_master_id = '5004' THEN
                           'Форс'
                          WHEN im.cd_master_id = '15004' THEN
                           'Смартмакс'
                          WHEN im.cd_master_id = '18005' THEN
                           'Орбико'
          ...
          JOIN item_master im ON im.sku_id = td.sku_id AND im.cd_master_id NOT IN ('3003', '9005', '9006', '11005', '18004', '10003')
          JOIN pkt_hdr ph ON ph.pkt_ctrl_nbr = td.pkt_ctrl_nbr
          JOIN pkt_hdr_intrnl phi ON phi.pkt_ctrl_nbr = ph.pkt_ctrl_nbr
         WHERE ((th.task_desc IN
               ('Отбор из ПС', 'Отбор из ПС паллет', 'Отбор из ПС Ростовки', 'Отбор LPN из ПС') AND substr(lh.area, 1, 1) IN ('P', 'X')) OR
               (ph.cd_master_id = '2001' AND (substr(ph.shipto_name, 1, 5) = 'Деком' OR ph.shipto_name = 'Д KYI Internet-Інтертоп'))) AND
               th.stat_code < '90')
 GROUP BY dep
 ORDER BY 3
